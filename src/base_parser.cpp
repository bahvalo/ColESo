...
#ifndef ASSERT
#include "base_lib.h" // crash, InformKGB
#include "base_io.h"  // IO_Open - встроенная обёртка от fopen
#endif
...
#if defined(EXTRAPRECISION_COLESO) || defined(EXTRAPRECISION)
#ifdef _NOISETTE
#include "module_TOYPROBLEMS/extraprecision.h"
#else
...
void GetNextLine(string &sbuf,FILE* pf,bool fullLine){ 
    ...
    }while(!fullLine && IsCommentLine(sbuf.c_str()));
...
//=====================================================================================================================
// Split <source> to vector of strings <elems>
//=====================================================================================================================
...
//======================================================================================================================
// Разделение строки на массив строк по разделителю comma
//======================================================================================================================
void ParseString(const string& S, vector<string>& VS, char comma) {
    char buf[8192];
    const char* s = S.c_str();
    while(*s) {
        char* pb = buf;
        while(*s && *s!=comma) *(pb++) = *(s++);
        *pb = 0;
        string sbuf = string(buf);
        VS.push_back(trim_string(sbuf));
        if(*s) s++;
    }
}
//======================================================================================================================
...
int GetEnumValue(const string& word, string Dictionary) {
    ...
    while(GetNextWord(pname, Dictionary)){ //iterating all words in dictionary
        ...
        if(CompareWords(pname, word)){ //if word is found
            if(isFound) crash("Error in dictionary (%s): value %s occurred twice", Dictionary.c_str(), word.c_str());
...
int ProcessEnum(const string& word, const string& Dictionary){
    if(IsInteger(word.c_str())) {
        ...
        if(Dictionary.empty())  return value; // no check - responsibility is on lazy user who did not defined dictionary
        if(CheckEnumValue(value, Dictionary)) return value;
        return UNDEF_INT;
    }
    else {
...
// Converts dictionary (enums) to defines (for OpenCL defines)
string EnumToDefines(const char* Dictionary, const string prefix){
    string res = "";
    string pname, pcode;
    string D = Dictionary;

    while(GetNextWord(pname, D, 0/*keep case*/)){ //iterating all words in dictionary
        if(!GetNextWord(pcode, D)) return res;
        int code = GetIntFromWord(pcode.c_str());
        if(IS_UNDEF_INT(code)) crash("GetEnumName: dictionary \"%s\" is incorrect", Dictionary);
        res += stringprintf("#define %s%s %d\n", ((string)(prefix.empty()?"":(prefix+"_"))).c_str(),
            pname.c_str(), code);
    }
    return res;
}
...
//======================================================================================================================
string correctString(const char *s){
    string str = string(s);
    size_t i = 0;
    while(true){
        i = str.find("%", i);
...
bool tParameter::GetValue(string pline, bool log, tErrAccumulator* ErrAccumulator) {
    ...
    if(isSet && !canRedef) { IO_error_add("Double definition of parameter %s", parName.c_str()); return false; }
    ...
    case PARTYPE_INT:
        ...
        if(ErrAccumulator != NULL && ErrAccumulator->Count() > err) break; // an error has occurred
    ...
    case PARTYPE_WORDLIST:
    {
        ...
        while(GetNextElem(temp, pline, false /*no lowercase*/)) {
            if(!GetNextWord(word, temp, 0)) { IO_error_add("Parameter \"%s\" contains an empty word in the list", parName.c_str()); break; }
            trim_string(temp);
...
void parse_pairs_list(tFileBuffer & fb, vector< pair<string, int> >& list, tErrAccumulator* ErrAccumulator) {
    list.clear();
    while (!fb.EndOfZone()) {
        string word, line;
        pair<string, int> pair;
        fb.GetNextLine(line);
        if (GetNextWord(word, line, 0)) {
            pair.first = word;
            if (GetNextWord(word, line, 0)) pair.second = GetIntFromWord(word.c_str(), ErrAccumulator);
            else pair.second = UNDEF_INT;
            if (GetNextWord(word, line, 0)) IO_error_add("parse_pairs_list: too many values in the string (\"%s\")", word.c_str());
            list.push_back(pair);
        }
    }
}
...
// Печать неиспользованных неименованных аргументов для файла fileName
...
bool tMacroSubstitutionVector::Evaluate(bool log) {
    ...
    static const string EVAL_INT = "EVAL_INT ";
    ...
    do {
        ...
        if(remainder == size() - ready) {
            IO_error_add("tMacroSubstitutionVector::Evaluate: All macros can not be evaluated");
...
int tMacroSubstitutionVector::Substitution(string &line, bool log) {
    ...
    static const int MAX_COUNT = 1000; // счетчик максимального количества подстановок
    size_t posStart = 0;
    string oldline = line;
...
void tParamManager::ReadParamsFromFile(const string& fileName, bool log, bool CMDLine){
    ...
    for(size_t i=0; i < size(); i++)
        if(at(i).GetCrashIt()) CrashIt = IO_CRASH; // set IO_CRASH if any obligatory parameter is present
...
void tParamManager::ReadParamsFromBuffer(tFileBuffer& FB, bool log, bool CMDLine){
    ...
    // проход по командной строке - определяем или переопределяем параметры
    if(CMDLine) {
        for(size_t i = 0; i < size(); i++){
...
// Extracts parameters pairs <parName,parValue> to map<string, string>
void tParamManager::ExtractParametersPairs(tFileBuffer &fb, map<string, string>& par) {
    while(!fb.EndOfZone()) {
        string line, parName;
        fb.GetNextLine(line);
...
double tFormula::Evaluate(tFormulaBuiltinArgs args, int trn){
    SetArg("index", (double)args.index, trn);
    if(args.coo){ 
        SetArg("x", args.coo[0], trn); 
        SetArg("y", args.coo[1], trn); 
        SetArg("z", args.coo[2], trn);
    }
    SetArg("label", (double)args.label, trn);
    SetArg("dist", (double)args.dist, trn);
    return Evaluate(trn);
}

bool tFormula::Test(tFormulaBuiltinArgs args, int trn){
    double res = Evaluate(args, trn);
    return fabs(res) > 0.5;
}
...
void tParamManager::GetUnnamedPathFromCommandLine(std::string &path) {
    ...
    for(i = 0; i < size(); i++) {
        if(!(at(i).GetParName())[0]) { // находим неименованные
...
//---------------------------------------------------------------------------------------------------------------------
// Запретить\разрешить переопределение параметров
void tParamManager::SetRedef(bool redef) {
    for (size_t iparam = 0; iparam < size(); iparam++) at(iparam).SetRedef(redef);
}
